    ...
    *,
    ...
    for model_name, use_penalty in [
        ...
        threshold, tune_metrics = choose_threshold(
            ...
            beta=F_BETA,
            min_threshold=0.5 if use_penalty else None,
        )
        tune_pred = (tune_score >= threshold).astype(np.int8)
        validation_pred = (validation_score >= threshold).astype(np.int8)
        validation_metrics = metric_dict(y_validation, validation_pred, validation_score)
...
def select_model(results: list[Result]) -> tuple[Result | None, Result]:
    ...
    selected = None
    if accepted:
        penalty = [result for result in accepted if "penalty" in result.model]
        pool = penalty if penalty else accepted
        selected = max(
            pool,
            key=lambda item: (
                item.validation_metrics["f0.5"],
...
def plot_feature_importance(selected: Result, spec: FeatureSpec) -> Path | None:
    if selected is None or selected.model_object is None or not hasattr(selected.model_object, "feature_importances_"):
        return None
    path = OUT_FIGURE_DIR / "selected_feature_importance.png"
    imp = np.asarray(selected.model_object.feature_importances_, dtype=float)
    ...
    ax.barh(y, imp[order], color="#4a6fa5")
    ax.set_yticks(y, labels=names[order])
    ax.invert_yaxis()
    ax.set_xlabel("XGBoost importance")
    ax.set_title("Selected individual-gene feature importance")
    fig.savefig(path, dpi=300, bbox_inches="tight")
    plt.close(fig)
    return path
...
def write_reports(
    ...
    *,
    ...
    lines = [
        ...
        f"- Thresholds were selected only on the tune split, requiring specificity >= `{MIN_SPECIFICITY}` and maximizing F{F_BETA}.",
        "- The GSE144469 holdout was used once for final validation and acceptance.",
        "- A learned model was accepted only if holdout specificity >= 0.995, precision >= 0.90, and holdout F0.5 exceeded the best individual-gene heuristic baseline. No learned model passed.",
        "- The whole-dataset application is therefore exploratory: it uses the best learned but unaccepted model and compares it to the original conservative `TRD_minus_TRAB` score strategy.",
        "",
        "## Feature Summary",
        "",
        ...
        "",
        *selected_block,
        f"- Best individual-gene heuristic baseline: `{best_baseline.model}` with holdout F0.5 `{best_baseline.validation_metrics['f0.5']:.4f}`.",
        "",
        ...
        *full_block,
        ...
        f"- Full application comparison: `{OUT_TABLE_DIR / 'exploratory_best_unaccepted_full_dataset_prediction_overall.csv'}`",
        f"- Full false-positive audit: `{OUT_TABLE_DIR / 'exploratory_best_unaccepted_full_dataset_false_positive_overall.csv'}`",
    ...
    *{box-sizing:border-box} body{margin:0;background:var(--bg);color:var(--ink);font-family:Arial,Helvetica,sans-serif;line-height:1.5}
    ...
    method_steps = [
        ...
        f"Select thresholds on tune at specificity >= {MIN_SPECIFICITY}, maximizing F{F_BETA}.",
        "Accept only if holdout specificity >= 0.995, precision >= 0.90, and F0.5 beats the best individual-gene heuristic baseline.",
    ]
    ...
    html_parts = [
        ...
        f"<section><h2>Training and Test Protocol</h2>{method_html}<p class='note'>The full-dataset application is exploratory because no learned model passed the strict holdout acceptance rule.</p></section>",
    ...
    with h5py.File(input_h5ad, "r") as handle:
        ...
        if applied_full_model is not None and not args.no_full_apply:
            if exploratory:
                ...
                full_application_note = f"Accepted holdout model `{applied_full_model.model}` was applied to the full plus6 dataset."
            logging.info("Applying %s to full plus6 dataset", applied_full_model.model)
