...
def train_candidates(
    ...
    if not args.skip_hist_gradient:
        ...
        for rescue_round, rescue_threshold in [(6, 0.80), (7, 0.72), (8, 0.64)]:
            append_fixed_threshold_candidate(
                ...
                notes=f"Round {rescue_round}. Histogram-gradient recall-rescue threshold fixed at {rescue_threshold}; intended to test whether full-atlas recall can exceed 0.8 while estimated FP remains below 5%.",
...
def select_candidate_from_full_atlas(
    ...
    rows["target_estimated_fp_fraction_met_full_atlas"] = rows["estimated_fp_fraction_of_predictions"].astype(float) <= args.max_estimated_fp_fraction
    rows["target_met_full_atlas"] = rows["target_recall_met_full_atlas"] & rows["target_estimated_fp_fraction_met_full_atlas"]
    ...
    def key(row: pd.Series) -> tuple[float, ...]:
        ...
        target_met = recall >= args.target_recall and estimated_fp <= args.max_estimated_fp_fraction
        recall_margin_met = recall >= args.target_recall + args.target_recall_margin
        if target_met:
            return (1.0, float(recall_margin_met), -estimated_fp, recall, f1, precision, -predicted)
        recall_gap = max(0.0, args.target_recall - recall)
        fp_excess = max(0.0, estimated_fp - args.max_estimated_fp_fraction)
        return (0.0, -recall_gap, -fp_excess, f1, precision, -predicted)
...
def save_best_model(candidate: CandidateResult, spec: FeatureSpec, external_acceptance: pd.DataFrame) -> tuple[Path, Path | None]:
    ...
    if accepted:
        ...
        (examples / "run_external_test.sh").write_text(
            "#!/usr/bin/env bash\n"
            "set -euo pipefail\n"
            "/home/tanlikai/miniconda3/envs/rapids_sc_py310/bin/python run_gdtai_v3_trdc_nk_guard_classifier.py --external-h5ad \"$1\"\n",
            encoding="utf-8",
        )
    return best_path, selected_path


def promotion_acceptance(selected: CandidateResult, external_metrics: pd.DataFrame, fp: pd.DataFrame, recall: pd.DataFrame) -> pd.DataFrame:
    ...
    checks = {
        "external_NK_FP_lower_than_v2_high_purity": float(fp_pivot.loc[selected.name, "NK_cells"]) < float(fp_pivot.loc["v2_high_purity", "NK_cells"]),
...
def main() -> None:
    ...
    acceptance = promotion_acceptance(selected, external_metrics, fp, recall)
