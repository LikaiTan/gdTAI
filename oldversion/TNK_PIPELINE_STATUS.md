...
- Review the completed pre-merge per-GSE TCR audit to determine whether suspicious paired `TRA/TRB` coverage is already present in the individual raw H5ADs before merge
- Treat the validated raw-TCR rebuild package under `Integrated_dataset/tables/tcr_rebuild_phase4/` and `Integrated_dataset/figures/tcr_rebuild_phase4/` as the current review artifact for suspicious near-100% paired `TRA/TRB` datasets
- Treat validated-plus-user-approved TCR propagation as complete for these ten GSEs:
  - `GSE188620`
  - `GSE212217`
  - `GSE243572`
  - `GSE243905`
  ...
  - `GSE168859`
  - `GSE227709`
- Record that `GSE161918`, `GSE168859`, and `GSE227709` remain unscoreable for the original Phase 4 validation gate, but their repaired TCR fields were explicitly approved for propagation after direct coverage review
...
- Completed the pre-merge per-GSE TCR audit on the current raw H5AD inventory with integrated TCR metadata
- The pre-merge audit discovered `39` eligible per-GSE H5ADs, scored `25`, and marked `14` as structurally unscoreable because the canonical `TRAB`/`TRD` module genes were missing from the source feature space
- The pre-merge audit outputs were written under:
  - `Integrated_dataset/tables/premerge_tcr_audit/`
  - `Integrated_dataset/figures/premerge_tcr_audit/`
  - `Integrated_dataset/logs/premerge_tcr_audit_summary.md`
- Among scoreable raw per-GSE H5ADs, several datasets already show extremely high paired `TRA/TRB` coverage before merge, so the suspicious pattern cannot be explained by merge-only corruption alone
- Rebuilt raw TCR assignments from `downloads/<GSE>/suppl` for the ten flagged near-100%-coverage GSEs in dry-run mode, using sample-aware `sample_id + barcode_core` joins instead of barcode-only logic
- Validated the rebuilt TCR assignments on per-GSE normalized Phase 4 score space using the user-requested pattern gate:
  - `TRD > 0.3` has the lowest paired `TRA/TRB` coverage
  - `TRD < 0.1` has the highest paired `TRA/TRB` coverage
  - paired `TRA/TRB` cells shift upward on `TRAB`
- Seven GSEs passed that validation gate:
  - `GSE188620`
  - `GSE212217`
  - `GSE243572`
  - `GSE243905`
  ...
  - `GSE308075`
  - `GSE311112`
- Three flagged GSEs were repairable at the raw-TCR join level but remain unscoreable for the requested validation because their per-GSE H5ADs lack the Phase 4 `TRA` module genes:
  - `GSE161918`
  - `GSE168859`
  - `GSE227709`
- Wrote the validated per-GSE TCR replacements and refreshed per-GSE Phase 4 validation package to:
  - `Integrated_dataset/tables/tcr_rebuild_phase4/`
  - `Integrated_dataset/figures/tcr_rebuild_phase4/`
  - `Integrated_dataset/logs/tcr_rebuild_phase4_qc.md`
- Propagated validated-only TCR columns into:
...
- The validated-only propagation left unresolved/unvalidated GSEs blank in the merged and integrated milestones rather than forcing a replacement
- The user later explicitly approved propagation for the three repaired-but-unscoreable GSEs `GSE161918`, `GSE168859`, and `GSE227709` after reviewing their rebuilt coverage values
- Re-ran `replace_validated_tcr_in_project_objects.py` with explicit include-gse overrides and propagated TCR columns for all ten approved GSEs into:
  - `analysis_26GSE_V4/outputs/harmonized_metadata_v4.csv`
  - `Integrated_dataset/TNK_candidates.h5ad`
  - `Integrated_dataset/TNK_cleaned.h5ad`
  - `high_speed_temp/Integrated_dataset/integrated.h5ad`
...
- [x] Audit the raw pre-merge per-GSE H5ADs for TRAB-vs-TRD score behavior and paired `TRA/TRB` coverage before merge
- [x] Rebuild raw TCR assignments for the flagged near-100%-coverage GSEs and validate them with the requested TRD/TRAB pattern check
- [ ] Review the three unresolved unscoreable GSEs (`GSE161918`, `GSE168859`, `GSE227709`) and decide whether they should remain excluded from merged/integrated TCR propagation or be handled with a different validation rule
...
- `audit_premerge_gse_tcr_score_scatter.py`
...
- `Integrated_dataset/tables/premerge_tcr_audit/premerge_tcr_audit_manifest.csv`
- `Integrated_dataset/tables/premerge_tcr_audit/premerge_tcr_audit_batches.csv`
- `Integrated_dataset/tables/premerge_tcr_audit/premerge_tcr_audit_gse_matrix_state.csv`
- `Integrated_dataset/tables/premerge_tcr_audit/premerge_tcr_audit_gse_score_summary.csv`
- `Integrated_dataset/tables/premerge_tcr_audit/premerge_tcr_audit_gse_paired_coverage.csv`
- `Integrated_dataset/tables/premerge_tcr_audit/premerge_tcr_audit_suspicious_ranked.csv`
- `Integrated_dataset/logs/premerge_tcr_audit_summary.md`
- `Integrated_dataset/figures/premerge_tcr_audit/`
...
- The pre-merge per-GSE TCR audit compiled and ran successfully in `rapids_sc_py310`
- The audit discovered `39` eligible raw H5ADs with TCR metadata, wrote `25` per-GSE scatter plots, and marked `14` datasets as unscoreable because the canonical `TRAB`/`TRD` module genes were absent before merge
- The lowest paired-coverage scoreable raw datasets include:
  - `GSE162498`, `GSE190870`, `GSE267645`, `GSE301528` at `0%`
  - `GSE287301` at `2.40%`
  - `GSE287541` at `32.93%`
- Many other scoreable raw datasets already show near-100% paired `TRA/TRB` coverage before merge, including `GSE188620`, `GSE243572`, `GSE243905`, `GSE254249`, `GSE212217`, `GSE311112`, and `GSE308075`
...
- Suspiciously high paired `TRA/TRB` coverage is already present in many scoreable raw per-GSE H5ADs before merge, so merge alone cannot explain the pattern
- The next diagnostic step should focus on pre-merge TCR metadata provenance and join logic, especially same-core-barcode reuse across different sample labels and sample-level TCR assignment leakage

---

## Git status
- Milestone documentation and workflow scripts were previously pushed through later Phase 4 work
- The pre-merge TCR audit script and status update are currently local changes and have not been committed yet

---

## Next action
- Review the pre-merge per-GSE TCR audit outputs with the user
- Use the audit result to decide whether the suspicious paired `TRA/TRB` signal is more consistent with source-level TCR metadata leakage, barcode collision across samples, or a smaller subset of genuinely paired datasets
- If requested, run the next source-provenance checks on same-core barcode reuse, cross-sample TCR sharing, or sample-level TCR metadata joins before trusting paired `TRA/TRB` coverage biologically
