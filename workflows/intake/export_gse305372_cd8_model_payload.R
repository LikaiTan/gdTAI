...
main <- function() {
  ...
  subset_column <- if (is.null(args[["subset-column"]])) "cite.cell.type.tag" else args[["subset-column"]]
  subset_value <- if (is.null(args[["subset-value"]])) "CD8A" else args[["subset-value"]]
  ...
  if (length(existing) > 0 && !overwrite) {
    stop(paste0("Output exists; pass --overwrite true to replace: ", paste(existing, collapse = ", ")))
  }
  ...
  if (!subset_column %in% colnames(object@meta.data)) {
    stop(sprintf("Required author annotation column is missing: %s", subset_column))
  }

  author_labels <- as.character(object@meta.data[[subset_column]])
  keep <- !is.na(author_labels) & author_labels == subset_value
  selected_cells <- rownames(object@meta.data)[keep]
  if (length(selected_cells) == 0) {
    stop(sprintf("No cells have %s == %s", subset_column, subset_value))
  }

  feature_table <- read.csv(feature_manifest, stringsAsFactors = FALSE, check.names = FALSE)
  model_genes <- feature_table$gene[
    feature_table$feature_type == "gene_log1p_cp10k" &
      !is.na(feature_table$gene) & feature_table$gene != ""
  ]
  if (anyDuplicated(model_genes)) {
    stop("Model feature manifest contains duplicated genes")
  }
  ...
  detected_genes <- as.integer(Matrix::colSums(counts > 0))

  metadata <- clean_metadata(object@meta.data[selected_cells, , drop = FALSE])
  metadata$cell_id_original <- selected_cells
  metadata$cell_id <- paste(compartment, selected_cells, sep = ":")
  metadata$gse_id <- "GSE305372"
  metadata$source_compartment <- compartment
  metadata$author_cd8_subset_column <- subset_column
  metadata$author_cd8_subset_value <- subset_value
  ...
  if ("umap" %in% Reductions(object)) {
    umap <- Embeddings(object, reduction = "umap")[selected_cells, , drop = FALSE]
  ...
  if ("nCount_RNA" %in% colnames(metadata)) {
    ncount_max_abs_diff <- max(abs(as.numeric(metadata$nCount_RNA) - total_counts), na.rm = TRUE)
  }
  ...
  writeLines(model_genes, paste0(output_prefix, "_model_genes.tsv"), useBytes = TRUE)
  ...
  manifest <- data.frame(
    ...
    selected_cd8_cells = length(selected_cells),
  ...
  write.csv(manifest, paste0(output_prefix, "_export_manifest.csv"), row.names = FALSE, quote = TRUE)

  cat(sprintf(
    "Exported %s: %s / %s author-labeled CD8 cells, %s model genes\n",
    compartment,
    format(length(selected_cells), big.mark = ","),
