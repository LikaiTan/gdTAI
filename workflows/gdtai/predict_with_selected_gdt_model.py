...
log1p-CP10K gene features used at training time, applies the saved threshold,
and applies FOXP3/CD4/low-CD3 death penalties only for saved model variants that
were trained with that post-processing rule. It then writes prediction tables and
a small static report.
"""
...
def setup_dirs(dataset_id: str) -> tuple[Path, Path, Path, Path]:
    table_dir = _TNK_PROJECT_ROOT / "Integrated_dataset/tables/gdT_prediction/external_tests" / dataset_id
    figure_dir = _TNK_PROJECT_ROOT / "Integrated_dataset/figures/gdT_prediction/external_tests" / dataset_id
    log_dir = _TNK_PROJECT_ROOT / "Integrated_dataset/logs/gdT_prediction/external_tests" / dataset_id
    static_dir = _TNK_PROJECT_ROOT / "gdT_prediction/external_tests" / dataset_id
...
def write_report(
    *,
    ...
    lines = [
        ...
        "- This is an F1-optimized gdT recovery classifier, not a clinical classifier.",
        "",
        "## Outputs",
    ]
...
def main() -> None:
    ...
    logging.info("Saved model name: %s", model_name)
    logging.info("Apply external FOXP3/CD4/low-CD3 death penalty: %s", apply_penalty_rule)
    ...
    with h5py.File(args.input_h5ad, "r") as handle:
        ...
        for start in range(0, n_obs, args.chunk_size):
            ...
            if apply_penalty_rule:
                ...
                chunk_penalty = np.zeros(chunk_raw.shape[0], dtype=bool)
