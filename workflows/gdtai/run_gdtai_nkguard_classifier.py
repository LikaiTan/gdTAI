...
"""Train and audit a gdTAI NK-guard candidate model.

This lane is separate from the current shared gdTAI model. It adds conservative
NK-annotated negatives as a training/tuning stressor, holds out the largest
current NK-FP sources as untouched NK-stress validation, and evaluates whether a
new model can reduce NK false positives without overfitting or losing too much
gdT recall.
"""
...
REPORT_MD = LOG_DIR / "gdtai_nkguard_report.md"
REPORT_HTML = STATIC_DIR / "gdtai_nkguard_report.html"
REPORT_PDF = STATIC_DIR / "gdtai_nkguard_report.pdf"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train gdTAI NK-guard candidate models.")
    ...
    parser.add_argument("--report-only", action="store_true", help="Rebuild HTML/PDF/protocol from existing NK-guard output tables and figures.")
...
def build_feature_spec(handle: h5py.File, *, include_nk_controls: bool, name: str) -> FeatureSpec:
    ...
    spec = FeatureSpec(
        ...
        family_by_feature=[nk_family(gene) for gene in selected],
...
def build_splits(handle: h5py.File, seed: int) -> tuple[Any, SplitBundle]:
    ...
    # Conservative NK negatives: annotation-only stress labels, excluding gdT evidence and silver/gold gdT cells.
    nk_guard_mask = (
        is_nk_annotation
        & (obs.class_code != 2)
        & (~obs.silver_mask)
    ...
    nk_train_parts: list[np.ndarray] = []
    nk_tune_parts: list[np.ndarray] = []
    nk_df = pd.DataFrame({"obs_index": nk_train_tune_pool, "source_gse_id": obs.source[nk_train_tune_pool]})
    for source, group in nk_df.groupby("source_gse_id", sort=True):
        idx = group["obs_index"].to_numpy(dtype=np.int64)
        rng.shuffle(idx)
        n_tune = 0 if idx.size < 5 else max(1, int(round(idx.size * 0.20)))
        if n_tune:
            nk_tune_parts.append(idx[:n_tune])
            nk_train_parts.append(idx[n_tune:])
        else:
            nk_train_parts.append(idx)
        rows.append({"source_gse_id": source, "label": "NK_guard_negative", "n_cells": int(idx.size), "train": int(idx.size - n_tune), "tune": int(n_tune)})
    nk_train_idx = np.concatenate(nk_train_parts).astype(np.int64)
    nk_tune_idx = np.concatenate(nk_tune_parts).astype(np.int64)

    split_by_source = pd.DataFrame(rows)
    ...
    conflict = int((nk_guard_mask & ((obs.class_code == 2) | obs.silver_mask | obs.corrected_has_any_gd_tcr)).sum())
    pd.DataFrame([{"nk_guard_conflict_with_gdT_evidence": conflict}]).to_csv(TABLE_DIR / "nk_guard_label_conflict_audit.csv", index=False)
    if conflict:
        raise RuntimeError(f"NK guard label conflict detected: {conflict}")
...
def sample_train_positions(pos_train_primary: np.ndarray, pos_train_nk: np.ndarray, y_eval: np.ndarray, is_nk_eval: np.ndarray, *, max_negative: int, max_nk: int, seed: int) -> np.ndarray:
    rng = np.random.default_rng(seed)
    ...
    out = np.concatenate([positives, primary_neg, nk_neg]).astype(np.int64)
    rng.shuffle(out)
    return out
...
def choose_guarded_threshold(primary_y: np.ndarray, primary_score: np.ndarray, nk_score: np.ndarray, *, current_primary_tune_f1: float) -> tuple[float, dict[str, Any], float, int, bool, str]:
    _fpr, _tpr, thresholds = roc_curve(primary_y, primary_score)
    thresholds = thresholds[np.isfinite(thresholds)]
    if thresholds.size == 0:
        thresholds = np.asarray([float(np.nanmax(primary_score) + 1e-6)], dtype=float)
    rows = []
    min_f1 = PRIMARY_TUNE_F1_FRACTION * current_primary_tune_f1
    for threshold in thresholds:
        primary_pred = primary_score >= threshold
        if primary_pred.sum() == 0:
            continue
        metrics = metric_dict(primary_y, primary_pred.astype(np.int8), primary_score)
        nk_predicted = int((nk_score >= threshold).sum())
        nk_fpr = safe_div(nk_predicted, nk_score.size)
        valid = bool((metrics["f1"] >= min_f1) and (nk_fpr <= NK_TUNE_FPR_MAX))
        rows.append((valid, metrics["f1"], -nk_fpr, metrics["precision"], metrics["recall"], float(threshold), metrics, nk_fpr, nk_predicted))
    if not rows:
        ...
        return threshold, metrics, float("nan"), 0, False, "no_nonempty_primary_predictions"
    valid_rows = [row for row in rows if row[0]]
    pool = valid_rows if valid_rows else rows
    best = max(pool, key=lambda row: (row[1], row[2], row[3], row[4], row[5]))
    reason = "valid_guarded_threshold" if best[0] else "fallback_no_threshold_met_NK_FPR_and_primary_F1_constraints"
    return best[5], best[6], best[7], best[8], bool(best[0]), reason


def train_candidates(x_base: np.ndarray, x_nk: np.ndarray, y_eval: np.ndarray, is_nk_guard_eval: np.ndarray, positions: dict[str, np.ndarray], current_payload: dict[str, Any], base_spec: FeatureSpec, nk_spec: FeatureSpec, args: argparse.Namespace) -> list[Candidate]:
    ...
    current_nk_tune_score = score_model(current_model, x_base[nk_tune])
    current_validation_score = score_model(current_model, x_base[primary_validation])
    current_nk_stress_score = score_model(current_model, x_base[nk_stress])
    train_sample = sample_train_positions(primary_train, nk_train, y_eval, is_nk_guard_eval, max_negative=args.max_negative_train, max_nk=args.max_nk_guard_train, seed=args.seed)
    logging.info(
        "NK-guard training sample: %s cells, positives=%s, primary_negatives=%s, nk_negatives=%s",
        f"{train_sample.size:,}",
        f"{int(y_eval[train_sample].sum()):,}",
        f"{int(((y_eval[train_sample] == 0) & (~is_nk_guard_eval[train_sample])).sum()):,}",
        f"{int(is_nk_guard_eval[train_sample].sum()):,}",
    )
    rows = [
        {
            "sample_class": "gdT_positive",
            "n_cells": int(y_eval[train_sample].sum()),
        },
        {
            "sample_class": "primary_abT_negative",
            "n_cells": int(((y_eval[train_sample] == 0) & (~is_nk_guard_eval[train_sample])).sum()),
        },
        {
            "sample_class": "NK_guard_negative",
            "n_cells": int(is_nk_guard_eval[train_sample].sum()),
        },
    ]
    pd.DataFrame(rows).to_csv(TABLE_DIR / "training_sample_composition.csv", index=False)
    ...
    def add_candidate(name: str, feature_set: str, spec: FeatureSpec, model_object: Any, x: np.ndarray, notes: str) -> None:
        ...
        threshold, tune_metrics, nk_tune_fpr, nk_tune_predicted, valid, reason = choose_guarded_threshold(
            y_eval[primary_tune], tune_score, nk_tune_score, current_primary_tune_f1=current_primary_tune_f1
        )
        ...
        nk_stress_fpr = safe_div(nk_stress_predicted, nk_stress_score.size)
        ...
        candidates.append(
            Candidate(
                ...
                nk_stress_fpr=float(nk_stress_fpr),
    ...
    add_candidate(
        "current_model_threshold_only_NK_guarded",
        "base_TCR_controls",
        base_spec,
        current_model,
        x_base,
        "Current shared logistic model with a retuned NK-guard threshold; comparator only, not promotable.",
    )

    logistic_base = make_pipeline(
        StandardScaler(),
        LogisticRegression(solver="lbfgs", class_weight="balanced", max_iter=1000, random_state=args.seed),
    )
    logistic_base.fit(x_base[train_sample], y_eval[train_sample])
    add_candidate(
        "logistic_TCR_controls_NK_guard_negatives",
        "base_TCR_controls",
        base_spec,
        logistic_base,
        x_base,
        "Balanced logistic model using TCR plus FOXP3/CD4/CD3 controls and NK-guard negatives.",
    )
    ...
    add_candidate(
        "logistic_TCR_NKmarker_controls_NK_guard_negatives",
        "TCR_NKmarker_controls",
        nk_spec,
        logistic_nk,
        x_nk,
        "Balanced logistic model using TCR, FOXP3/CD4/CD3, fixed NK marker controls, and NK-guard negatives.",
    )
    ...
    xgb.fit(x_nk[train_sample], y_eval[train_sample])
    add_candidate(
        "xgb_TCR_NKmarker_controls_NK_guard_negatives",
        "TCR_NKmarker_controls",
        nk_spec,
        xgb,
        x_nk,
        "XGBoost model using TCR, FOXP3/CD4/CD3, fixed NK marker controls, and NK-guard negatives.",
    )
    return candidates
...
def candidate_table(candidates: list[Candidate]) -> pd.DataFrame:
    ...
    return pd.DataFrame(rows).sort_values(["threshold_valid", "primary_tune_f1", "nk_tune_fpr"], ascending=[False, False, True]).reset_index(drop=True)


def select_tune_candidate(candidates: list[Candidate]) -> Candidate:
    learned = [c for c in candidates if not c.model.startswith("current_model_")]
    valid = [c for c in learned if c.threshold_valid]
    pool = valid if valid else learned
    return max(pool, key=lambda c: (bool(c.threshold_valid), c.primary_tune_metrics["f1"], -c.nk_tune_fpr, c.primary_tune_metrics["precision"]))
...
def apply_candidate_full(handle: h5py.File, candidate: Candidate) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, np.ndarray]:
    ...
    for start in range(0, n_obs, FULL_APPLY_CHUNK):
        ...
        x = extract_gene_matrix(handle, rows, candidate.spec, label=f"nkguard_full_{start}_{end}")
        score = score_model(candidate.model_object, x)
        pred[start:end] = score >= candidate.threshold
        if start and start % 500_000 == 0:
            logging.info("Applied NK-guard candidate to %s / %s cells", f"{start:,}", f"{n_obs:,}")
    ctx = full_context(handle)
    return (*summarize_full_predictions(
        strategy=f"nkguard_candidate_{candidate.model}",
...
def combine_with_current_outputs(candidate_outputs: tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    fp_cols = list(current["fp_overall"].columns)
    cand_fp_overall = cand_overall[[col for col in fp_cols if col in cand_overall.columns]].copy()
    overall = pd.concat([cand_overall, current["overall"]], ignore_index=True)
    source = pd.concat([cand_source, current["source"]], ignore_index=True)
    tissue = pd.concat([cand_tissue, current["tissue"]], ignore_index=True)
    fp_overall = pd.concat([cand_fp_overall, current["fp_overall"]], ignore_index=True)
    fp_source = pd.concat([cand_fp_source, current["fp_source"]], ignore_index=True)
    annotation = pd.concat([cand_annotation, current["annotation"]], ignore_index=True)
...
def evaluate_acceptance(candidate: Candidate, fp_overall: pd.DataFrame) -> pd.DataFrame:
    cand_row = fp_overall.loc[fp_overall["strategy"].astype(str).str.startswith("nkguard_candidate_")].iloc[0]
    m = candidate.primary_validation_metrics
    nk_reduction = 1.0 - safe_div(candidate.nk_stress_fpr, candidate.current_nk_stress_fpr)
    checks = {
...
def plot_candidate_summary(candidate_df: pd.DataFrame, acceptance: pd.DataFrame, fp_overall: pd.DataFrame) -> list[Path]:
    ...
    ax.bar(x - 0.2, plot["primary_validation_f1"], width=0.4, label="validation F1", color="#0f766e")
    ax.bar(x + 0.2, 1.0 - plot["nk_stress_fpr"], width=0.4, label="1 - NK stress FPR", color="#1d4ed8")
    ax.set_xticks(x)
    ax.set_xticklabels(plot["model"], rotation=25, ha="right", fontsize=8)
    ax.set_ylim(0, 1.05)
    ax.set_title("NK-guard candidate validation summary")
    ...
    comp["label"] = comp["strategy"].map(lambda v: "NK-guard" if str(v).startswith("nkguard_candidate_") else ("Current gdTAI" if str(v).startswith("selected_model_") else "Original TRD-TRAB"))
    comp = comp.drop_duplicates("label")
    ax.bar(comp["label"], comp["predicted_NK_FP"], color=["#0f766e" if x == "NK-guard" else "#64748b" for x in comp["label"]])
    for i, value in enumerate(comp["predicted_NK_FP"]):
        ax.text(i, value, f"{int(value):,}", ha="center", va="bottom", fontsize=9)
    ax.set_ylabel("NK false-positive count")
    ax.set_title("Full-atlas NK false positives")
    path = FIGURE_DIR / "full_atlas_nk_fp_comparison.png"
    fig.savefig(path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    paths.append(path)
...
def plot_trd_trab_scatter(handle: h5py.File, candidate: Candidate, candidate_pred_full: np.ndarray, current_payload: dict[str, Any], base_spec: FeatureSpec, seed: int, sample_cells: int) -> Path:
    ...
    logging.info("Building NK-guard TRD-vs-TRAB scatter sample with %s cells", f"{sample_idx.size:,}")
    x_candidate = extract_gene_matrix(handle, sample_idx, candidate.spec, label="nkguard_trd_trab_scatter")
    ...
    category[candidate_pred & (~current_pred)] = "NK-guard only"
    category[current_pred & candidate_pred] = "both"
    df = pd.DataFrame({"phase4_trab_score": trab, "phase4_trd_score": trd, "category": category})
    df.to_csv(TABLE_DIR / "trd_vs_trab_current_vs_nkguard_scatter_sample.csv.gz", index=False)
    fig, ax = plt.subplots(figsize=(7.2, 6.0), constrained_layout=True)
    xlim, ylim = bounded_scatter_limits(trab, trd)
    colors = {"neither": "#c4c9ce", "current only": "#d97904", "NK-guard only": "#2b7c9b", "both": "#7a3db8"}
    for cat in ["neither", "current only", "NK-guard only", "both"]:
    ...
    ax.set_title("TRD vs TRAB score space: current gdTAI vs NK-guard")
    ax.legend(frameon=False, fontsize=7)
    path = FIGURE_DIR / "trd_vs_trab_current_vs_nkguard.png"
...
def write_report(candidate: Candidate, candidate_df: pd.DataFrame, acceptance: pd.DataFrame, full_overall: pd.DataFrame, fp_overall: pd.DataFrame, fp_source: pd.DataFrame, split_overall: pd.DataFrame, figures: list[Path], best_path: Path, selected_path: Path | None) -> None:
    accepted = bool(acceptance["accepted_for_promotion"].iloc[0])
    cand_full = fp_overall.loc[fp_overall["strategy"].astype(str).str.startswith("nkguard_candidate_")].iloc[0]
    ...
    nk_reduction = 1.0 - safe_div(float(cand_full["predicted_NK_FP"]), float(current_full["predicted_NK_FP"]))
    summary_lines = [
        "# gdTAI NK-Guard Candidate Report",
        "",
        ...
        f"- Full-atlas NK FP: `{int(cand_full['predicted_NK_FP']):,}` vs current `{int(current_full['predicted_NK_FP']):,}` (delta `{nk_delta:,}`, relative `{nk_reduction:.2%}`)",
        ...
        "",
        "- The NK-guard approach is useful for suppressing NK false positives, but this candidate is not the promoted shared gdTAI model.",
        "- It passed the primary validation, NK-stress reduction, full-atlas NK FP, and known-FP fraction gates.",
        "- It failed the paired-TCRAB FP gate: 3,108 paired-TCRAB cells were predicted as gdT, above the preset limit of 2,600.",
        "- The current selected gdTAI model remains the model to share for now; the NK-guard pickle is kept as a candidate artifact for review.",
        "- A next attempt should include paired-TCRAB false positives directly in threshold selection or multi-objective model selection, not only as a final acceptance gate.",
        "",
        "## Split Summary",
        ...
        "",
        "## Top NK FP Sources",
        dataframe_to_markdown(fp_source.sort_values("predicted_NK_FP", ascending=False).head(25)),
        "",
    ]
    ...
    for path in figures:
        ...
        fig_blocks.append(
            f"<section class='figure'><h3>{html.escape(path.stem.replace('_', ' '))}</h3><img src='assets/{OUT_PREFIX}/{html.escape(path.name)}'></section>"
        )
    fig_html = "\n".join(fig_blocks)
    status_color = "#0f766e" if accepted else "#be123c"
    css = """
    @page{size:A4 landscape;margin:8mm} body{font-family:Arial,Helvetica,sans-serif;margin:18px;color:#1d252c;line-height:1.45;background:#f5f6f7} main{max-width:1320px;margin:auto} section{background:white;border:1px solid #d9dee4;padding:16px;margin:12px 0;break-inside:avoid} h1{font-size:29px;margin:0 0 8px} h2{font-size:20px} table{border-collapse:collapse;width:100%;font-size:9px;table-layout:fixed} th,td{border:1px solid #d9dee4;padding:3px 4px;text-align:left;vertical-align:top;overflow-wrap:anywhere} th{background:#eef1f4}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}.metric{background:white;border:1px solid #d9dee4;padding:12px}.value{font-size:23px;font-weight:bold}.figure img{max-width:100%;height:auto;border:1px solid #d9dee4;background:white}.status{font-weight:bold;color:__STATUS_COLOR__} code{background:#eef1f4;padding:1px 4px;border-radius:3px}
    """.replace("__STATUS_COLOR__", status_color)
    html_doc = f"""<!doctype html><html><head><meta charset='utf-8'><title>gdTAI NK-Guard Report</title><style>{css}</style></head><body><main>
    <section><h1>gdTAI NK-Guard Candidate Report</h1><p class='status'>Promotion accepted: {accepted}</p><p>This report trains NK-guard candidates using NK annotations only for conservative negative labels and audits, not as prediction features.</p></section>
    ...
    <section><h2>Interpretation And Decision</h2><p>The NK-guard candidate strongly reduced NK false positives, but it is not promoted because the paired-TCRAB false-positive count exceeded the preset gate. The current selected gdTAI model remains the shared model; this pickle is kept as a candidate artifact for review.</p><p>Next iteration should tune against paired-TCRAB false positives and NK false positives together, instead of using paired-TCRAB only as a final acceptance gate.</p></section>
    ...
    <section><h2>Top NK FP Sources</h2>{table_html(fp_source.sort_values('predicted_NK_FP', ascending=False), max_rows=25)}</section>
...
def write_protocol(candidate: Candidate, accepted: bool, best_path: Path, selected_path: Path | None) -> Path:
    ...
    text = f"""# gdTAI NK-Guard Candidate Usage Protocol

This model was trained to reduce NK false positives by adding conservative NK-annotated negatives during training and threshold tuning. NK annotation is not an inference feature.
...
def feature_spec_from_payload(payload: dict[str, Any]) -> FeatureSpec:
    ...
    penalty = [gene for gene in gene_names if gene in PENALTY_CONTROL_GENES or gene in NK_CONTROL_GENES]
    return FeatureSpec(
        feature_names=feature_names,
        gene_names=gene_names,
        gene_indices=np.arange(len(gene_names), dtype=np.int32),
        tcr_gene_names=[gene for gene in gene_names if gene.upper().startswith(TCR_PREFIXES)],
        penalty_gene_names=penalty,
        family_by_feature=[nk_family(gene) for gene in gene_names],
        cd3_cols=[gene_to_col[gene] for gene in ["CD3D", "CD3E", "CD3G"] if gene in gene_to_col],
        cd4_col=gene_to_col.get("CD4"),
        foxp3_col=gene_to_col.get("FOXP3"),
    )


def validation_metrics_from_row(row: pd.Series) -> dict[str, Any]:
    tp = int(row["primary_validation_tp"])
    fp = int(row["primary_validation_fp"])
    tn = int(row["primary_validation_tn"])
    fn = int(row["primary_validation_fn"])
    n_positive = tp + fn
    n_negative = tn + fp
    return {
        "n_cells": n_positive + n_negative,
        ...
        "tn": tn,
        "fn": fn,
        "precision": float(row["primary_validation_precision"]),
        "recall": float(row["primary_validation_recall"]),
        "specificity": float(row["primary_validation_specificity"]),
        "f1": float(row["primary_validation_f1"]),
        "f0.5": float("nan"),
...
def report_only(args: argparse.Namespace) -> None:
    ...
    if not best_path.exists():
        raise FileNotFoundError(f"Missing saved candidate model: {best_path}")
    with best_path.open("rb") as fh:
        payload = pickle.load(fh)
    candidate_df = pd.read_csv(TABLE_DIR / "candidate_tune_validation_metrics.csv")
    acceptance = pd.read_csv(TABLE_DIR / "promotion_acceptance_gates.csv")
    full_overall = pd.read_csv(TABLE_DIR / "full_dataset_prediction_overall.csv")
    fp_overall = pd.read_csv(TABLE_DIR / "full_dataset_false_positive_overall.csv")
    fp_source = pd.read_csv(TABLE_DIR / "full_dataset_false_positive_by_source.csv")
    split_overall = pd.read_csv(TABLE_DIR / "split_overall.csv")
    model_name = str(payload["model"])
    row_match = candidate_df.loc[candidate_df["model"].astype(str) == model_name]
    if row_match.empty:
        raise RuntimeError(f"Saved model {model_name!r} is absent from candidate metrics table.")
    row = row_match.iloc[0]
    candidate = Candidate(
        model=model_name,
        feature_set=str(payload.get("feature_set", row["feature_set"])),
        spec=feature_spec_from_payload(payload),
        model_object=payload.get("model_object"),
        threshold=float(payload["threshold"]),
        threshold_valid=bool(row["threshold_valid"]),
        threshold_reason=str(row["threshold_reason"]),
        primary_tune_metrics={
            "f1": float(row["primary_tune_f1"]),
            "precision": float(row["primary_tune_precision"]),
            "recall": float(row["primary_tune_recall"]),
        },
        nk_tune_fpr=float(row["nk_tune_fpr"]),
        nk_tune_predicted=int(row["nk_tune_predicted"]),
        current_primary_tune_f1=float(row["current_primary_tune_f1"]),
        primary_validation_metrics=validation_metrics_from_row(row),
        nk_stress_fpr=float(row["nk_stress_fpr"]),
        nk_stress_predicted=int(row["nk_stress_predicted"]),
        current_nk_stress_fpr=float(row["current_nk_stress_fpr"]),
        current_nk_stress_predicted=int(row["current_nk_stress_predicted"]),
        notes=str(payload.get("notes", row.get("notes", ""))),
    )
    selected_path = MODEL_DIR / "selected_model.pkl"
    if not selected_path.exists():
        selected_path = None
    figures = [
        FIGURE_DIR / "candidate_validation_summary.png",
        FIGURE_DIR / "full_atlas_nk_fp_comparison.png",
        FIGURE_DIR / "promotion_gate_result.png",
        FIGURE_DIR / "selected_candidate_confusion_matrix.png",
        FIGURE_DIR / "trd_vs_trab_current_vs_nkguard.png",
    ]
    missing = [str(path) for path in figures if not path.exists()]
    if missing:
        raise FileNotFoundError("Missing report figures: " + ", ".join(missing))
    accepted = bool(acceptance["accepted_for_promotion"].iloc[0])
    protocol_path = write_protocol(candidate, accepted, best_path, selected_path)
    write_report(candidate, candidate_df, acceptance, full_overall, fp_overall, fp_source, split_overall, figures, best_path, selected_path)
    render_pdf(args.no_pdf)
    (LOG_DIR / "gdtai_nkguard_summary.json").write_text(
        json.dumps(
            {
                "report_only": True,
                "candidate_model": candidate.model,
...
def main() -> None:
    ...
    if args.report_only:
        report_only(args)
        logging.info("Rebuilt NK-guard report from existing outputs: %s", REPORT_HTML)
        return
    ...
    with h5py.File(input_h5ad, "r") as handle:
        ...
        eval_rows = np.unique(np.concatenate([splits.train_idx, splits.tune_idx, splits.validation_idx, splits.nk_train_idx, splits.nk_tune_idx, splits.nk_stress_idx])).astype(np.int64)
        positions = {
        ...
        logging.info("Extracting NK-control feature matrix for %s rows", f"{eval_rows.size:,}")
        x_nk = extract_gene_matrix(handle, eval_rows, nk_spec, label="nkguard_train_tune_validation")
        x_base = x_nk[:, : len(base_spec.gene_names)]
        candidates = train_candidates(x_base, x_nk, y_eval, is_nk_eval, positions, current_payload, base_spec, nk_spec, args)
        ...
        full_overall, full_source, full_tissue, fp_overall, fp_source, full_annotation = combine_with_current_outputs(candidate_full_tuple)
    ...
    if (stat_before.st_size != stat_after.st_size) or (stat_before.st_mtime_ns != stat_after.st_mtime_ns):
        raise RuntimeError("Input H5AD changed during NK-guard read-only training/audit.")
    write_report(
        selected_candidate,
        candidate_df,
        acceptance,
    ...
    logging.info("Saved NK-guard report: %s", REPORT_HTML)
    logging.info("Saved NK-guard PDF: %s", REPORT_PDF)
